...
pub fn pixy(b: &mut Board) -> ! {
    use core::fmt::Write;
    b.display.row_0().write_fmt(format_args!("hw v: {}", b.pixy.version.hardware)).ok();
    b.display.row_1().write_fmt(format_args!("fw v: {}.{}",
        b.pixy.version.firmware_major, b.pixy.version.firmware_minor)).ok();  
    rprintln!("frame height: {}, frame width: {}", b.pixy.frame_height, b.pixy.frame_width);
    loop {
        b.delay.delay_ms(1u8);
        if !b.pixy.get_blocks(false, crate::pixy2::SigMap::ALL, 10).is_err() {
            continue;
        }
        for i in 0..b.pixy.num_blocks as usize {
            let block = b.pixy.blocks[i];
            rprintln!("sig: {}, x: {}, y: {}, width: {}, height: {}, angle: {}, index: {}, age: {}",
                block.signature, block.x, block.y, block.width, block.height,
                block.angle, block.index, block.age);
        }
    }
}